...
const QUERY = gql`
  query ($mangaDetailId: String!, $offset: Int = 0) {
    mangaDetail(id: $mangaDetailId, offset: $offset) {
      description
      title
...
export default function MangaDetail() {
  ...
  const { data, loading } = useQuery(QUERY, {
    ...
    onCompleted: () => {
      const x = Number(data.mangaDetail.totalVolumes / 25).toFixed(2);
      const y = Number(data.mangaDetail.totalVolumes / 25).toFixed(0);
      const p = [];
      if (x !== y) {
